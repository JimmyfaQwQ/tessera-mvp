...
/// Error: `await <signal>` used outside an async function / handler.
/// L-AWAIT-ASYNC-ONLY already covers all bare `await` in sync context, but
/// surfacing a signal-specific message helps users discover `.wait()`.
pub struct SignalAwaitInSync;

impl LintPass for SignalAwaitInSync {
    fn name(&self) -> &'static str { "L-SIGNAL-AWAIT-IN-SYNC" }
        ...
        if !self.in_async {
            if let Expr::Await(a) = e {
                if let Expr::Ident(i) = &a.expr {
                    if self.env.lookup(&i.name) == Some(&Type::Signal) {
                        self.diags.push(
                            Diagnostic::error(
                                "L-SIGNAL-AWAIT-IN-SYNC",
                                "`await <signal>` is only allowed inside an async function or handler",
                                a.span,
                            )
