...
/// Warn: `.wait()` on a contract inside an async function blocks the thread.
/// Mirror of `SignalWaitInAsync` for `Type::Contract`.
pub struct ContractWaitInAsync;

impl LintPass for ContractWaitInAsync {
    fn name(&self) -> &'static str { "L-CONTRACT-WAIT-IN-ASYNC" }
        ...
        if self.in_async {
            if let Expr::MethodCall(m) = e {
                if m.method.name == "wait" {
                    if matches!(self.typer.receiver_type(m), Some(Type::Contract)) {
                        self.diags.push(
                            Diagnostic::warn(
                                "L-CONTRACT-WAIT-IN-ASYNC",
                                ".wait() on a contract in async context blocks the thread; use `await c` instead",
                                m.span,
                            )
                        );
                    }
