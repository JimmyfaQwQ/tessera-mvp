//! L-TERMINATE-FUTURE-IGNORED (Warn): a bare `handle.terminate();` statement
//! drops the returned `Future<void>` without awaiting it, so the caller does not
//! wait for teardown to finish (《线程与事件循环规范 §3》).
//!
//! Owns the `terminate` method so it never double-reports with
//! L-AWAIT-UNCONSUMED-FUTURE (which skips `terminate`). Receiver types are
//! resolved with `ScopedTyper`, mirroring `handler_result_ignored`.
...
pub struct TerminateFutureIgnored;

impl LintPass for TerminateFutureIgnored {
    fn name(&self) -> &'static str { "L-TERMINATE-FUTURE-IGNORED" }
            ...
            Stmt::Expr(es) => {
                if let Expr::MethodCall(m) = &es.expr {
                    if m.method.name == "terminate" {
                        if let Some(Type::ThreadHandle(_)) = self.typer.receiver_type(m) {
                            self.diags.push(
                                Diagnostic::warn(
                                    "L-TERMINATE-FUTURE-IGNORED",
                                    "the Future returned by `terminate()` is dropped; teardown is not awaited",
                                    m.span,
                                )
                                .with_help("`.wait()` (sync) or `await` (async) the terminate Future to wait for teardown to finish"),
                            );
                        }
                    }
                }
